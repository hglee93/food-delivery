...
1. 고객이 선생님 매칭 요청을 한다.
1. 고객이 결제한다
1. 매칭 요청이 되면 매칭요청 내역이 선생님에게 전달된다
1. 선생님이 확인하여 선생님 배정 및 방문 시간을 설정하여 방문을 확정한다.
1. 고객이 방문을 취소할 수 있다
1. 주문이 취소되면 선생님 방문이 취소된다
1. 고객이 매칭 요청 상태에 대해 중간중간 조회한다.

비기능적 요구사항
1. 트랜잭션
    1. 결제가 되지 않은 매칭 요청건은 아예 선생님 방문이 성립되지 않아야 한다  Sync 호출 
1. 장애격리
    1. 방문관리 기능이 수행되지 않더라도 매칭 요청은 365일 24시간 받을 수 있어야 한다  Async (event-driven), Eventual Consistency
    1. 결제시스템이 과중되면 사용자를 잠시동안 받지 않고 결제를 잠시후에 하도록 유도한다  Circuit breaker, fallback
1. 성능
    1. 고객이 자주 방문 관리에서 확인할 수 있는 배달상태를 주문시스템(프론트엔드)에서 확인할 수 있어야 한다  CQRS
